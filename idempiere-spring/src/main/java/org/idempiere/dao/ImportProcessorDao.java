...
public interface ImportProcessorDao extends CrudDao<ImportProcessor, Integer, SQLBuilder.NSC, ImportProcessorDao> {
    ...
    @Query("SELECT * FROM IMP_Processor WHERE Value = :value AND AD_Client_ID = :clientId")
    Optional<ImportProcessor> findByValue(String value, int clientId);

    @Query("SELECT * FROM IMP_Processor WHERE Name = :name AND AD_Client_ID = :clientId")
    Optional<ImportProcessor> findByName(String name, int clientId);

    @Query("SELECT * FROM IMP_Processor WHERE IMP_Processor_Type_ID = :processorTypeId AND IsActive = 'Y' ORDER BY Name")
    List<ImportProcessor> findByType(int processorTypeId);

    @Query("SELECT * FROM IMP_Processor WHERE IsActive = 'Y' AND DateNextRun <= :now ORDER BY DateNextRun")
    List<ImportProcessor> findDueForRun(LocalDateTime now);

    @Query("SELECT * FROM IMP_Processor WHERE AD_Client_ID = :clientId AND IsActive = 'Y' ORDER BY Name")
