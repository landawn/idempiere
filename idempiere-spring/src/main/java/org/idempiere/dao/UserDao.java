 ...
 */
public interface UserDao extends CrudDao<User, Integer, SQLBuilder.NSC, UserDao> {

    /**
     * Find a user by email
     */
    @Select("SELECT * FROM AD_User WHERE Email = :email AND IsActive = 'Y'")
    Optional<User> findByEmail(String email);

    /**
     * Find a user by name
     */
    @Select("SELECT * FROM AD_User WHERE Name = :name AND IsActive = 'Y'")
    Optional<User> findByName(String name);

    /**
     * Find a user by value (login name)
     */
    @Select("SELECT * FROM AD_User WHERE Value = :value AND IsActive = 'Y'")
    Optional<User> findByValue(String value);

    /**
     * Find all users for a specific business partner
     */
    @Select("SELECT * FROM AD_User WHERE C_BPartner_ID = :bPartnerId AND IsActive = 'Y' ORDER BY Name")
    List<User> findByBPartnerId(Integer bPartnerId);

    /**
     * Find all active users for a client
     */
    @Select("SELECT * FROM AD_User WHERE AD_Client_ID = :clientId AND IsActive = 'Y' ORDER BY Name")
    List<User> findByClientId(Integer clientId);

    /**
     * Find all sales representatives
     */
    @Select("SELECT u.* FROM AD_User u " +
     ...
     */
    @Select("SELECT * FROM AD_User WHERE IsLocked = 'N' AND IsActive = 'Y' ORDER BY Name")
