...
public interface TranslationDao extends CrudDao<MessageTrl, Integer, SQLBuilder.NSC, TranslationDao> {

    @Select("SELECT * FROM AD_Message_Trl WHERE AD_Message_ID = :messageId AND AD_Language = :language")
    Optional<MessageTrl> findMessageTrl(int messageId, String language);

    @Select("SELECT * FROM AD_Message_Trl WHERE AD_Language = :language AND IsTranslated = 'Y'")
    List<MessageTrl> findMessagesByLanguage(String language);

    @Select("SELECT * FROM AD_Element_Trl WHERE AD_Element_ID = :elementId AND AD_Language = :language")
    Optional<ElementTrl> findElementTrl(int elementId, String language);

    @Select("SELECT * FROM AD_Element_Trl WHERE AD_Language = :language AND IsTranslated = 'Y'")
    List<ElementTrl> findElementsByLanguage(String language);

    @Select("SELECT * FROM AD_Window_Trl WHERE AD_Window_ID = :windowId AND AD_Language = :language")
    Optional<WindowTrl> findWindowTrl(int windowId, String language);

    @Select("SELECT * FROM AD_Menu_Trl WHERE AD_Menu_ID = :menuId AND AD_Language = :language")
